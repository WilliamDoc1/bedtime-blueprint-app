...
const TableOfContents: React.FC = () => {
  return (
    <div className="container mx-auto p-4 bg-lavender-50 rounded-lg shadow-md">
      <h1 className="text-4xl font-bold mb-8 text-gold-500 text-center">Guide: Table of Contents</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {chapterDetails.map((chapter) => (
          <Card key={chapter.number} className="hover:shadow-lg transition-shadow duration-300 bg-white border-gold-400 border-2">
            <CardHeader>
              <CardTitle className="text-xl font-semibold text-gold-500">Chapter {chapter.number}: {chapter.title}</CardTitle>
            </CardHeader>
            <CardContent>
